let sketch = function (p) {
        ...
        if (noiseMapMode === 1) {
            ...
            for (let x = 0; x < p.width; x += noiseGridSize * 2) {
                for (let y = 0; y < p.height; y += noiseGridSize * 2) {
                    let angle;
                    if (drawMode === 1) {
                        angle = p.noise(x / noiseScale, y / noiseScale) * noiseStrength;
                    } else {
                        angle = p.noise(x / noiseScale, y / noiseScale) * 24;
                        angle = (angle - Math.floor(angle)) * noiseStrength;
                    }
            ...
            for (let x = 0; x < p.width; x += noiseGridSize * 3) {
                for (let y = 0; y < p.height; y += noiseGridSize * 3) {
                    let angle;
                    if (drawMode === 1) {
                        // カールノイズの場合はベクトルから角度を計算
                        let curl = curlNoise(x / noiseScale, y / noiseScale);
                        angle = p.atan2(curl.y, curl.x); // ベクトルから角度を取得

                        let magnitude = curl.mag();
                        let hue = p.map(magnitude, 0, noiseStrength, 0, 360) % 360;
                        p.colorMode(p.HSB, 360, 100, 100, 255);
                        ...
                        let hue = p.map(angle, 0, noiseStrength, 0, 360) % 360;
                        p.colorMode(p.HSB, 360, 100, 100, 255);
                        p.stroke(hue, 80, 80, noiseMapAlpha);
                    }
    ...
    function curlNoise(x, y) {
        // (2Dの場合) パーリンノイズ N(x, y) をそのままポテンシャル関数とする
        let n = p.noise(x, y) * noiseStrength;

        // 2D curlの計算を行う
        // 速度場は流れのストリーム関数として、ポテンシャルの勾配の90度回転で定義される
        let dx = p.noise((x + 0.01), y) * noiseStrength - n;
        let dy = p.noise(x, (y + 0.01)) * noiseStrength - n;
        let curl = p.createVector(dy, -dx); // 90度回転

        // 非圧縮性の再現
        // どんな滑らかなポテンシャルから計算しても自動的に発散がゼロになるようにする
        let length = curl.mag();
        if (length > 0) {
            curl.x /= length;
            curl.y /= length;
        }
        curl.mult(noiseStrength); // 強さを調整
